...
#include <esp_now.h>
#include <WiFi.h>
//#include <debug.h> //? Is this necessary? 
...
int SM_state = 0; // This'll record the current state in the state machine. Our initial state is state 0.
...
unsigned long wait_after_fcam_enabled = 3000; // 
...
void IRAM_ATTR reverseSignal(){
  ...
  if (digitalRead(reverseSignalpin))  reverseEngaged = 1;
  //Reverse gear was disengaged:
  else  reverseEngaged = 0;
}
